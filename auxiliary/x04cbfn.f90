...
!     .. Parameters ..
      ...
      Character (6), Parameter :: srname = 'X04CBF'
!     .. Scalar Arguments ..
...
!     Check for incorrect arguments.
      If (.Not. generl .And. .Not. lower .And. matrix/='U' .And. matrix/='u') &
        ...
        ierr = 1
        nrec = 1
        Write (rec, Fmt=180) matrix
        Write (errbuf, Fmt=190) srname, 1, 99999, 1, matrix
      Else If ((labrow/='N' .And. labrow/='n' .And. labrow/='I' .And. &
        ...
        ierr = 6
        nrec = 2
        Write (rec, Fmt=240) labrow, labcol
        Write (errbuf, Fmt=250) srname, 6, 99996, 2, labrow, labcol
      Else If (m>lda) Then
        ierr = 3
        nrec = 1
        Write (rec, Fmt=200) m, lda
        Write (errbuf, Fmt=210) srname, 3, 99998, 2, m, lda
      Else If (.Not. generl) Then
        If (diag/='U' .And. diag/='u' .And. diag/='N' .And. diag/='n' .And. &
          diag/='B' .And. diag/='b') Then
          ierr = 2
          nrec = 1
          Write (rec, Fmt=220) matrix, diag
          Write (errbuf, Fmt=230) srname, 2, 99997, 2, matrix, diag
        End If
      End If
      If (ierr/=0) Go To 170
!
...
!        The length of FORM is too great.
        ierr = 4
        nrec = 2
        Write (rec, Fmt=260) form(start:start+74)
        Write (errbuf, Fmt=270) srname, 4, 99995, 1, form(start:start+74)
        Go To 170
      End If
!
...
!     The format in FMAT caused an error when used to print a number.
      ierr = 5
      nrec = 2
      Write (rec, Fmt=280) form(1:min(75,len(form)))
      Write (errbuf, Fmt=290) srname, 5, 99994, 2, form(1:min(75,len(form)))
      Go To 170
110   Continue
!
...
!
      ...
      If (nelems<1) Then
        ierr = 7
        nrec = 2
        Write (rec, Fmt=300) incols + indnt, indnt
        Write (errbuf, Fmt=310) srname, 7, 99993, 2, incols + indnt, indnt
        Go To 170
      End If
...
!           General rectangular matrix.
            Write (infile(indnt+rlbwid+2:), Fmt=fmat, Err=120)(a(j,cl), &
...
!                 out when CLEFT.GT.CRIGHT.
                  Write (infile(indnt+rlbwid+2:), Fmt=fmat, Err=130)(a(j,cl), &
                    cl=cleft, cright), diagel
                Else
                  Write (infile(indnt+rlbwid+2:), Fmt=fmat, Err=130) diagel
                End If
130             Continue
              Else
...
!             Upper triangular matrix.
              If (prdiag) Then
                Write (infile(indnt+rlbwid+2+numwid*(nd- &
                  1):), Fmt=fmat, Err=150) diagel, (a(j,cl), cl=cleft, cright)
150             Continue
              Else
                If (cleft<=cright) Then
...
!                 out of range.
                  Write (infile(indnt+rlbwid+2+numwid*nd:), Fmt=fmat, Err=160) &
...
!
      Return
!
180   Format (' ** On entry, MATRIX is not valid: MATRIX = ''', A, '''.')
190   Format (A, 3(1X,I5), 1X, A)
200   Format (' ** On entry, M .gt. LDA: M = ', I16, ', LDA = ', I16, '.')
210   Format (A, 3(1X,I5), 1X, I16, 1X, I16)
220   Format (' ** On entry, MATRIX = ''', A, ''', but DIAG is not val', &
        'id: DIAG = ''', A, '''.')
230   Format (A, 3(1X,I5), 1X, A, 1X, A)
240   Format (' ** On entry, either LABROW or LABCOL is not valid:', /, 4X, &
        'LABROW = ''', A, ''', LABCOL = ''', A, '''.')
250   Format (A, 3(1X,I5), 1X, A, 1X, A)
260   Format (' ** On entry, FORM has more than 80 characters: the', &
        ' first 75 are', /, 4X, A, '.')
270   Format (A, 3(1X,I5), 1X, A)
280   Format (' ** The format specifier in FORM cannot be used to ', &
        'print a number: FORM =', /, 4X, A, '.')
290   Format (A, 3(1X,I5), 1X, A)
300   Format (' ** On entry, NCOLS-INDENT is not wide enough to hold', &
