...
!     .. Parameters ..
      ...
      Character (6), Parameter :: srname = 'X04DFF'
!     .. Scalar Arguments ..
...
!     Check for incorrect arguments.
      If (kl<0) Then
        ierr = 1
        nrec = 1
        Write (rec, Fmt=160) kl
        Write (errbuf, Fmt=170) srname, 1, 99999, 1, kl
      Else If (ku<0) Then
        ierr = 2
        nrec = 1
        Write (rec, Fmt=180) ku
        Write (errbuf, Fmt=190) srname, 2, 99998, 1, ku
      Else If ((labrow/='N' .And. labrow/='n' .And. labrow/='I' .And. &
        ...
        ierr = 7
        nrec = 2
        Write (rec, Fmt=220) labrow, labcol
        Write (errbuf, Fmt=230) srname, 7, 99996, 2, labrow, labcol
      Else If (.Not. above .And. .Not. brack .And. usefrm/='D' .And. &
          usefrm/='d') Then
        ierr = 4
        nrec = 1
        Write (rec, Fmt=240) usefrm
        Write (errbuf, Fmt=250) srname, 4, 99995, 1, usefrm
      Else If (lda<kl+ku+1) Then
        ierr = 3
        nrec = 2
        Write (rec, Fmt=200) lda, kl, ku
        Write (errbuf, Fmt=210) srname, 3, 99997, 3, lda, kl, ku
      End If
      If (ierr/=0) Go To 150
!
...
!       ignoring zeros.
        ...
        Do j = 1, n
          ...
          Do i = max(1, j-ku), min(m, j+kl)
            aa = max(abs(real(a(k+i,j))), abs(aimag(a(k+i,j))))
...
!       The length of FORM is too great.
        ...
        Write (errbuf, Fmt=270) srname, 5, 99994, 1, form(start:start+74)
        Go To 150
      End If
!
      If (brack) Then
...
!     The format in FMAT caused an error when used to print a number.
      ...
      Write (errbuf, Fmt=290) srname, 6, 99993, 2, form(1:min(75,len(form)))
      Go To 150
110   Continue
!
...
!
      ...
      If (nelems<1) Then
        ...
        Write (errbuf, Fmt=310) srname, 8, 99992, 2, incols + indnt, indnt
        Go To 150
      End If
...
!                 If we are in the non-zero band.
              If (above) Then
                Write (infile(offset+1:offset+numwid), Fmt=fmat, Err=120) &
                  real(a(ku+j-k+1,k))
120             Continue
                Write (infbot(offset+1:offset+numwid), Fmt=fmat, Err=130) &
                  aimag(a(ku+j-k+1,k))
130             Continue
              Else
                Write (infile(offset+1:offset+numwid), Fmt=fmat, Err=140) &
                  a(ku+j-k+1, k)
140             Continue
              End If
            End If
...
!
      Return
!
160   Format (' ** On entry, KL .lt. 0: KL =', I16, '.')
170   Format (A, 3(1X,I5), 1X, I16)
180   Format (' ** On entry, KU .lt. 0: KU =', I16, '.')
190   Format (A, 3(1X,I5), 1X, I16)
200   Format (' ** On entry, LDA .lt. KL+KU+1: LDA =', I16, ',', /, 4X, &
        'KL =', I16, ', KU =', I16, '.')
210   Format (A, 3(1X,I5), 1X, I16, 1X, I16, 1X, I16)
220   Format (' ** On entry, either LABROW or LABCOL is not valid', /, 4X, &
        'LABROW = ''', A, ''', LABCOL = ''', A, '''.')
230   Format (A, 3(1X,I5), 1X, A, 1X, A)
240   Format (' ** On entry, invalid USEFRM: USEFRM = ''', A, '''.')
